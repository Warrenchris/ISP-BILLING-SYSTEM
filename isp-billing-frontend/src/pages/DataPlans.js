...
import {
  DataUsage as DataUsageIcon, Speed as SpeedIcon,
  Schedule as ScheduleIcon, Star as StarIcon, Add as AddIcon,
...
const DataPlans = () => {
  const { dataPlansApi, subscriptionsApi } = useApi();
  const { user } = useAuth();
  ...
  const getCategoryConfig = (cat) => {
    const configs = {
      ...
      standard: { 
        color: 'primary', 
        gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
        bgGradient: 'linear-gradient(135deg, rgba(25, 118, 210, 0.1) 0%, rgba(21, 101, 192, 0.1) 100%)',
      ...
      premium: { 
        ...
        bgGradient: 'linear-gradient(135deg, rgba(156, 39, 176, 0.1) 0%, rgba(233, 30, 99, 0.1) 100%)',
      ...
      enterprise: { 
        color: 'warning', 
        // FIXED: Changed to darker, more visible gradient for enterprise
        gradient: 'linear-gradient(135deg, #ff6b35 0%, #f7931e 100%)',
        bgGradient: 'linear-gradient(135deg, rgba(255, 107, 53, 0.15) 0%, rgba(247, 147, 30, 0.15) 100%)',
        borderColor: '#ff6b35',
  ...
  const confirmSubscription = async () => {
    ...
    try {
      setSubscribing(true);
      await subscriptionsApi.create({ planId: selectedPlan.id });
      showAlert('Successfully subscribed to plan!', 'success');
      setConfirmDialog(false);
      setSelectedPlan(null);
    } catch (err) {
      console.error(err);
      showAlert(err.response?.data?.message || 'Failed to subscribe', 'error');
    } finally {
      ...
      >
        <CardContent sx={{ flexGrow: 1, p: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
            <Box sx={{ width: '60%', height: 20, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
            <Box sx={{ width: 50, height: 20, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
          </Box>
          <Box sx={{ width: '40%', height: 16, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1, mb: 2 }} />
          <Box sx={{ width: '70%', height: 32, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1, mb: 2 }} />
          <Stack spacing={0.5}>
            <Box sx={{ width: '100%', height: 14, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
            <Box sx={{ width: '90%', height: 14, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
            <Box sx={{ width: '95%', height: 14, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
          </Stack>
        </CardContent>
        <CardActions sx={{ p: 2, pt: 0 }}>
          <Box sx={{ width: '100%', height: 32, bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1 }} />
        </CardActions>
      </Card>
    </Fade>
  );
                ...
                <Fade in={true} timeout={300 + index * 100}>
                  <Card
                    ...
                    sx={{
                      ...
                      background: isHovered ? config.bgGradient : 'rgba(255, 255, 255, 0.9)',
                      backdropFilter: 'blur(20px)',
